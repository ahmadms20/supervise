...
function TableAbsenAdminDashboard({api, token}) {
    ...
    const handleSearch = (e) => {
        ...
        if (e.target.value != '') {
            ...
            .then(res => {
                if (res.data.data != 'Empty') {
                    let data = []
                    res.data.data.map(item => data.push({nip: item.nip, status_absen: item.status_absen, pegawai: {nama: item.nama}}))
                    setItems(data)
                }
            })
        } else {
...
export default TableAbsenAdminDashboard;