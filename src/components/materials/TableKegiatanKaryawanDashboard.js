...
function TableKegiatanKaryawanDashboard({api, token}) {
    ...
    const handleSearch = (e) => {
        ...
        if (e.target.value != '') {
            ...
            .then(res => {
                if (res.data.data != 'Empty') {
                    let data = []
                    res.data.data.map(item => data.push({nip: item.nip, pegawai: {nama: item.nama}, kegiatan: item.kegiatan}))
                    setItems(data)
                }
            })
        } else {
...
export default TableKegiatanKaryawanDashboard;